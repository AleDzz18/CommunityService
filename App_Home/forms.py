...
class FormularioPerfilUsuario(forms.ModelForm):
    ...
    es_admin_basura = forms.BooleanField(
        required=False, label="Optar por Líder de Basura"
    )
    es_admin_clap = forms.BooleanField(required=False, label="Optar por Líder de CLAP")
    es_admin_bombonas = forms.BooleanField(
        required=False, label="Optar por Líder de Bombonas"
    )
    ...
    def clean(self):
        ...
        for field_name, role_name in secondary_roles:
            ...
            if is_checked:
                ...
                if query.exists():
                    self.add_error(
                        None,
                        f"Error de Permisos: El rol secundario '{role_name}' ya está asignado a otro usuario. Solo se permite un líder por gestión.",
                    )
        ...
        if rol == ROL_LIDER_TORRE:
            ...
            if not tower:
                self.add_error(
                    "tower",
                    "Debes seleccionar una Torre obligatoriamente para ser Líder de Torre.",
                )
            else:
                ...
                query = CustomUser.objects.filter(
                    rol=ROL_LIDER_TORRE, tower=tower
                ).exclude(
                ...
                if query.exists():
                    ...
                    self.add_error(
                        "tower",
                        f"La Torre {tower_name} ya tiene un Líder de Torre asignado. Por favor, selecciona otra Torre o Rol.",
                    )
    ...
    def clean_rol(self):
        ...
        if rol == ROL_LIDER_GENERAL and self.instance.rol != ROL_LIDER_GENERAL:
            conteo_lideres = (
                CustomUser.objects.filter(rol=ROL_LIDER_GENERAL)
            ...
            if conteo_lideres >= 2:
                raise ValidationError(
                    "Ya existe el número máximo de 2 Líderes Generales permitidos. Por favor, selecciona otro rol."
                )
    ...
    def save(self, commit=True):
        ...
        user.es_admin_basura = self.cleaned_data.get(
            "es_admin_basura", user.es_admin_basura
        )
        user.es_admin_clap = self.cleaned_data.get("es_admin_clap", user.es_admin_clap)
        user.es_admin_bombonas = self.cleaned_data.get(
            "es_admin_bombonas", user.es_admin_bombonas
        )
...
class SolicitudDocumentoForm(forms.Form):
    cedula = forms.CharField(
        ...
        widget=forms.TextInput(
            attrs={"class": "form-control", "placeholder": "Ej: 12.345.678"}
        ),
    )
