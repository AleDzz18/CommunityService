...
class Tower(models.Model):
    nombre = models.CharField(
        max_length=50, unique=True, verbose_name="Nombre de la Torre"
    )
...
class MovimientoFinancieroManager(models.Manager):
    def calcular_saldo_torre(self, tower, categoria):
        ...
        qs = self.filter(tower=tower, categoria=categoria)

        # Suma los ingresos
        ingresos = qs.filter(tipo="ING").aggregate(total=Sum(monto_field))[
            "total"
        ] or Decimal("0.00")

        # Suma los egresos
        egresos = qs.filter(tipo="EGR").aggregate(total=Sum(monto_field))[
            "total"
        ] or Decimal("0.00")
...
class MovimientoFinanciero(models.Model):
    ...
    monto_condominio = models.DecimalField(
        max_digits=10, decimal_places=2, default=0.00, verbose_name="Monto Condominio"
    )
    monto_basura = models.DecimalField(
        max_digits=10, decimal_places=2, default=0.00, verbose_name="Monto Basura"
    )

    tipo = models.CharField(
        max_length=3, choices=TIPOS, verbose_name="Tipo de Movimiento"
    )
    categoria = models.CharField(
        max_length=3, choices=CATEGORIAS, verbose_name="Categoría"
    )
    ...
    def __str__(self):
        return (
            f"{self.get_tipo_display()} - {self.get_categoria_display()} ({self.fecha})"
        )
    ...
    def calcular_saldo_condominio(tower=None):
        ...
        ingresos = (
            qs.filter(tipo="ING").aggregate(total=Sum("monto_condominio"))["total"] or 0
        )
        egresos = (
            qs.filter(tipo="EGR").aggregate(total=Sum("monto_condominio"))["total"] or 0
        )
        return ingresos - egresos
    ...
    def calcular_saldo_basura(tower=None):
        ...
        ingresos = (
            qs.filter(tipo="ING").aggregate(total=Sum("monto_basura"))["total"] or 0
        )
        egresos = (
            qs.filter(tipo="EGR").aggregate(total=Sum("monto_basura"))["total"] or 0
        )
        return ingresos - egresos
...
class CustomUser(AbstractUser):
    ...
    apartamento = models.CharField(
        max_length=10, null=True, blank=True, verbose_name="Apartamento (Ej: P1-A)"
    )

    # Campos de Roles Secundarios (Permisos)
    es_admin_basura = models.BooleanField(
        default=False, verbose_name="Administrador de Cuarto de Basura"
    )
    es_admin_clap = models.BooleanField(
        default=False, verbose_name="Administrador de Bolsas CLAP"
    )
    es_admin_bombonas = models.BooleanField(
    ...
    if sender.name == "App_Home":
        ...
        try:
            ...
            if "no such table" in str(e):
                print(
                    f"ADVERTENCIA: La tabla 'App_Home_tower' aún no existe. La creación de torres se pospone hasta que se complete la migración."
                )
            else:
...
class CensoMiembro(models.Model):
    ...
    # Opciones para Apartamentos (4 Pisos, 4 Letras)
    PISOS = [
        ("PB", "Planta Baja"),
        ("P1", "Piso 1"),
        ("P2", "Piso 2"),
    ...
    genero = models.CharField(
        max_length=1, choices=GENEROS, verbose_name="Género", default=None
    )
    ...
    apartamento_letra = models.CharField(
        max_length=1, choices=LETRAS, verbose_name="Letra Apto", default=None
    )

    es_jefe_familia = models.BooleanField(
...
class CicloBeneficio(models.Model):
    ...
    tipo = models.CharField(
        max_length=4, choices=TIPOS, verbose_name="Tipo de Beneficio"
    )
    nombre = models.CharField(
        max_length=50, verbose_name="Nombre del Ciclo (Ej: Noviembre 2024)"
    )
...
class EntregaBeneficio(models.Model):
    ciclo = models.ForeignKey(
        CicloBeneficio, on_delete=models.CASCADE, related_name="entregas"
    )
    beneficiario = models.ForeignKey(
        CensoMiembro, on_delete=models.CASCADE, verbose_name="Vecino Beneficiado"
    )
...
class SolicitudDocumento(models.Model):
    ...
    beneficiario = models.ForeignKey(
        CensoMiembro, on_delete=models.CASCADE, verbose_name="Vecino Solicitante"
    )

    tipo = models.CharField(
        max_length=30,
        choices=TIPOS,
        default="CARTA_CONDUCTA",
    ...
    mudanza_fecha_fin = models.CharField(
        max_length=50,
        blank=True,
        null=True,
