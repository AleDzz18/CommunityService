...
from .forms import (
    FormularioAdminUsuario,
    IngresoCondominioGeneralForm,
    EgresoCondominioGeneralForm,
    IngresoBasuraGeneralForm,
    EgresoBasuraGeneralForm,
    ProcesarCartaConductaForm,
    ProcesarCartaMudanzaForm,
    ProcesarConstanciaSimpleForm,
    ProcesarConstanciaMigratoriaForm,
...
class CrearUsuarioView(LoginRequiredMixin, LiderGeneralRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        messages.success(
            self.request, f"Usuario '{form.instance.username}' creado con éxito."
        )
...
class EditarUsuarioView(LoginRequiredMixin, LiderGeneralRequiredMixin, UpdateView):
    ...
    def form_valid(self, form):
        messages.success(
            self.request, f"Usuario '{form.instance.username}' editado con éxito."
        )
        return super().form_valid(form)
...
class EliminarUsuarioView(LoginRequiredMixin, LiderGeneralRequiredMixin, DeleteView):
    ...
    def form_valid(self, form):
        messages.success(
            self.request, f"Usuario '{self.object.username}' eliminado con éxito."
        )
        return super().form_valid(form)
...
class RegistrarEgresoCondominioGeneralView(
    LiderGeneralRequiredMixin, BaseMovimientoCreateView
):
...
class RegistrarIngresoBasuraGeneralView(
    LiderGeneralRequiredMixin, BaseMovimientoCreateView
):
...
class RegistrarEgresoBasuraGeneralView(
    LiderGeneralOrAdminBasuraRequiredMixin, BaseMovimientoCreateView
):
...
class CrearCicloView(LoginRequiredMixin, View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if request.user.rol == "LDG":
            permiso = True
        elif tipo == "CLAP" and request.user.es_admin_clap:
            permiso = True
        elif tipo == "GAS" and request.user.es_admin_bombonas:
            permiso = True
...
class AgregarBeneficiarioGeneralView(
    LoginRequiredMixin, LiderGeneralRequiredMixin, ListView
):
    model = CensoMiembro
    ...
    def post(self, request, tipo_slug):
        miembros_ids = request.POST.getlist(
            "miembros_ids"
        )  # Esperamos una lista de IDs de checkboxes

        if not miembros_ids:
            messages.error(request, "No seleccionaste a ningún miembro para agregar.")
            return redirect(
                "lider_general:agregar_beneficiario_global", tipo_slug=tipo_slug
            )
        ...
        try:
            ...
            messages.success(
                request,
                f"Se agregaron **{len(objetos_a_crear)}** miembros a la lista de {tipo_db}.",
            )
        except IntegrityError:
            messages.warning(
                request, "Algunos miembros ya estaban en la lista y fueron omitidos."
            )
...
class ProcesarSolicitudView(LoginRequiredMixin, UserPassesTestMixin, View):
    ...
    def get(self, request, pk):
        ...
        return render(
            request,
            "lider_general/solicitudes_procesar.html",
    ...
    def post(self, request, pk):
        ...
        if form.is_valid():
            ...
            if solicitud.tipo == "CARTA_MUDANZA":
                messages.success(
                    request,
                    f"Se ha procesado y generado el PDF de Carta de Mudanza para {solicitud.beneficiario.nombres}.",
                )
                ...
                messages.success(
                    request,
                    f"Se ha procesado y generado el PDF de Carta de Conducta para {solicitud.beneficiario.nombres}.",
                )
                ...
                messages.success(
                    request,
                    f"Se ha procesado y generado el PDF de Constancia de Residencia para {solicitud.beneficiario.nombres}.",
                )
                ...
                messages.success(
                    request,
                    f"Se ha procesado y generado el PDF de Constancia Migratoria para {solicitud.beneficiario.nombres}.",
                )
                ...
                messages.warning(
                    request,
                    "Documento procesado pero no se pudo generar el PDF (Tipo no implementado).",
                )
    ...
    def generar_pdf_conducta(self, solicitud):
        ...
        if solicitud.logo_clap == True:
            try:
                ...
                image_path = os.path.join(
                    settings.BASE_DIR, "App_Home", "static", "img", "clap_logo.png"
                )
        ...
        lider_calle_qs = CustomUser.objects.filter(
            rol="LDT", tower=solicitud.beneficiario.tower, is_active=True
        )
        lider_calle = lider_calle_qs.first()

        lc_nombre = (
            f"{lider_calle.first_name} {lider_calle.last_name}"
            if lider_calle
        ...
        Story.append(
            PDFTable(
                [
                    [
                        Paragraph("____________________", estilo_linea_firma),
                        Paragraph("____________________", estilo_linea_firma),
                    ]
                ],
                colWidths=[250, 250],
        ...
        Story.append(
            PDFTable(
                [
                    [
                        Paragraph(f"{jefe_nombre}", estilo_firmas),
                        Paragraph(f"{lc_nombre}", estilo_firmas),
                    ],
                    [
                        Paragraph(f"V- {jefe_cedula}", estilo_firmas),
                        Paragraph(f"V- {lc_cedula}", estilo_firmas),
                    ],
                    [
                        Paragraph("Jefe de Comunidad", estilo_firmas),
                        Paragraph("Líder de Calle", estilo_firmas),
                    ],
                ],
                colWidths=[250, 250],
            )
    ...
    def generar_pdf_residencia(self, solicitud):
        ...
        # TÍTULO DEL DOCUMENTO
        Story.append(
            Paragraph("<u>CONSTANCIA DE RESIDENCIA</u>", estilo_titulo_documento)
        )
        Story.append(Spacer(1, 12))
        ...
        Story.append(
            PDFTable(
                [
                    [
                        Paragraph(jefe_nombre, estilo_firmas),
                        Paragraph(lc_nombre, estilo_firmas),
                    ],
                    [
                        Paragraph(f"V- {jefe_cedula}", estilo_firmas),
                        Paragraph(f"V- {lc_cedula}", estilo_firmas),
...
class CensoPDFGeneralView(LoginRequiredMixin, LiderGeneralRequiredMixin, View):
    def get(self, request, *args, **kwargs):
        ...
        # Educación y Trabajo ahora están a la derecha de "Pe"
        headers = [
            Paragraph("Torre/Apto", style_header),
            ...
            Paragraph("Edad", style_header), # Ancho ajustado abajo
            Paragraph("Teléfono", style_header),
            Paragraph("Jefe", style_header), # Ancho ajustado abajo
        ...
        for m in miembros:
            ...
            data.append([
                Paragraph(f"{m.tower.nombre}-{m.apartamento_completo}", style_cell),
        ...
        # Aumenté Edad a 25 y Jefe a 25 para evitar el salto de línea en la cabecera
        col_widths = [
            45, # Torre
            58, # Cédula
            95, # Nombre
            18, # Sex
            48, # F. Nac
            25, # Edad (Suficiente para la palabra "Edad")
            62, # Teléfono
            25, # Jefe (Suficiente para la palabra "Jefe")
            15, # Tr
            15, # Es
            15, # Me
            15, # Pe
            ...
            80, # Instrucción
            80, # Lugar Trab
            171 # Salud (Aprovechando el resto del espacio)
        ]
        ...
        doc.build(Story)
        response = HttpResponse(content_type="application/pdf")
        response["Content-Disposition"] = f'attachment; filename="Censo_{timezone.now().strftime("%Y%m%d")}.pdf"'
...
class InventarioBasuraListView(ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if user.is_authenticated:
            ...
            context["es_admin_basura"] = (
                False  # Si no está logueado, nunca puede ser administrador.
            )
...
class InventarioBasuraUpdateView(
    LoginRequiredMixin, LiderGeneralOrAdminBasuraRequiredMixin, UpdateView
):
    model = InventarioBasura
    form_class = InventarioBasuraForm
    template_name = "lider_general/inventario_form.html"
...
class InventarioBasuraDeleteView(
    LoginRequiredMixin, LiderGeneralOrAdminBasuraRequiredMixin, DeleteView
):
    model = InventarioBasura
